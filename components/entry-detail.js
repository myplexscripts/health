...
import { backHeader, escapeHTML, formatDateTime, NUTRIENTS } from './shared.js?v=1.2.0';
  ...
  render({ params, store }) {
    ...
    return `
      ...
      <div class="ios-scroll"><div class="ios-content health-form-content">
        ...
        ${entry.type === 'food' ? `
          <section class="ios-section"><article class="ios-card health-entry-hero"><div class="health-entry-hero__icon health-icon--food"><span data-ios-symbol="utensils"></span></div><div><div class="ios-headline">${escapeHTML(entry.serving)}</div><div class="ios-subheadline ios-secondary">${escapeHTML(meal)}</div></div><strong>${Math.round(entry.calories)}<small>kcal</small></strong></article></section>
          <section class="ios-section"><div class="ios-section__header">Nutrition</div><article class="ios-card health-detail-grid">${nutritionMetrics || metric('Nutrients', 'Not recorded')}</article></section>` : `
          <section class="ios-section"><article class="ios-card health-detail-grid health-detail-grid--health">
            ...
            ${entry.readingType === 'heart-rate' ? `${metric('Heart rate', entry.heartRate, 'bpm')}${metric('Context', entry.context || 'Not recorded')}` : ''}
